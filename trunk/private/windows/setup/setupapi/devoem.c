...
Arguments:
    DeviceInfoSet - Supplies a handle to the device information set containing the device being installed.
...
Return Value:
    ...
    an ERROR_* code.
Remarks:
    This routine will allow browsing of local and network drives for OEM install files.
--*/
{
    ...
    try {
        ...
        if (!LoadString(MyDllModuleHandle, IDS_OEMTITLE, Title, SIZECHARS(Title))) {
            Title[0] = TEXT('\0');
        }
...
/*++
Routine Description:
    This routine selects a driver for a device using an OEM path supplied by the user.
Arguments:
    ...
    DeviceInfoData - Optionally, supplies the address of a SP_DEVINFO_DATA
        ...
        of the most-compatible driver found, if a compatible driver list was built.
Return Value:
    If the function succeeds (i.e., a driver is selected successfully), the return value is TRUE.
...
Arguments:
    ...
    DeviceInfoSet - Supplies the handle of the device info set for which an OEM driver selection is to be performed.
...
--*/
{
    ...
    try {
        if (DeviceInfoData) {
            ...
            // we're going to be calling the class installer, and we don't want to allow it to delete this element!
        ...
        // Don't assume there is no old OEM path.  Save old one and pretend there is no old one in case of cancel.
        ...
        while (bAskAgain && SetupDiAskForOEMDisk(DeviceInfoSet, DeviceInfoData)) {
            ...
            case ERROR_DI_DO_DEFAULT:
                // This case is only handled if we're in a wizard.  Otherwise, send it down for default processing.
                if (!IsWizard) {
                ...
                // If it is empty we will ask again, otherwise we will accept the new selection and go on.
                ...
                if (!NewClassDriverCount) {
                    ...
                    if (DevInfoElem && (DevInfoElem->InstallParamBlock.FlagsEx & DI_FLAGSEX_DIDCOMPATINFO)) {
    ...
    // If we need to restore any state, then we must make sure that we have the HDEVINFO locked.
