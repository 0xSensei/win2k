...
static HRESULT WINAPI SharedBuffer_Allocate(LPSTREAM pstm)
{
    ...
    while (!fFound && (nBufNum < BUFFERSINCACHE) && (cbSize <= CBBUFFERSIZE)) {
        ...
        fFound = (MyInterlockedCompareExchange((PVOID*)&BufferCache[nBufNum].m_fInUse, (PVOID)TRUE, FALSE) == FALSE);
...
//              a global memory handle that must be GlobalFree()'d by the caller.
// returns:
...
//  This gets an interface pointer from the object specified by dwId inside of the window.
STDAPI AccessibleObjectFromWindow(HWND hwnd, DWORD dwId, REFIID riid, void** ppvObject)
{
    ...
    *ppvObject = NULL;
    ...
    if (IsWindow(hwnd)) {
        ...
        SendMessageTimeout(hwnd, WM_GETOBJECT, wParam, dwId, SMTO_ABORTIFHUNG, 10000, &ref);
    }
...
//  # of chars so that he can turn around and allocate something the right size.
...
STDAPI CreateStdAccessibleObject(HWND hwnd, LONG idObject, REFIID riid, void** ppvObject)
{
    ...
    *ppvObject = NULL;
    ...
    default:
        ...
        return FindAndCreateWindowClass(hwnd, TRUE, NULL, riid, idObject, ppvObject);
    }
                                 ...
                                 IAccessible** ppacc, VARIANT* pvarChild)
{
    ...
    if (SUCCEEDED(hr) && pdispChild) {
        ...
        pacc->Release();// Release the parent.

        // Convert the child to an IAccessible*
        pacc = NULL;
        hr = pdispChild->QueryInterface(IID_IAccessible, (void**)&pacc);
        
        pdispChild->Release();// Release the IDispatch* form of the child
...
STDAPI AccessibleObjectFromPoint(POINT ptScreen, IAccessible** ppAcc, VARIANT* pvarChild)
{
    ...
    *ppAcc = NULL;
    ...
    hr = AccessibleObjectFromWindow(hwndPoint, OBJID_WINDOW, IID_IAccessible, (void**)&pAcc);
