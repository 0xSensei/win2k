...
int CEudcList::GetBarPosition(WORD     Code)
{
    ...
    for (int i = 1; i < Info.nTralByte; i++) {
        Space[i] = ((Info.sTralByte[i] & 0xf0) - (Info.eTralByte[i - 1] & 0xf0) - 0x10);
    }
...
WORD CEudcList::GetCodeScrPos(int     Pos)
{
    WORD    i;
    WORD    Code = 0;
    WORD    NumLine = 0;
...
/*    MESSAGE    "WM_VSCROLL"        */
void CEudcList::OnVScroll(UINT     nSBCode, UINT     nPos, CScrollBar * pScrollBar)
{
    int    MoveScr;
    BOOL    ThumbTrkFlag;
    BOOL    ThumbPosFlag;
...
void CEudcList::OnLButtonDown(UINT     nFlags, CPoint     point)
{
    CRect    test;
    BOOL    PtIn;
    unsigned int    i, j;
...
void CEudcList::OnLButtonDblClk(UINT     nFlags, CPoint     point)
{
    BOOL     PtIn;
    unsigned int    i, j;
...
BOOL CEudcList::OnSetCursor(CWnd * pWnd, UINT     nHitTest, UINT     message)
{
...
void CEudcList::DrawConcave(CDC * dc, CRect     rect, BOOL     PtIn)
{
...
/*    MESSAGE    "WM_KEYDOWN"        */
void CEudcList::OnKeyDown(UINT     nChar, UINT    nRepCnt, UINT     nFlags)
{
    int    sPos;
    int    ePos;
#ifdef BUILD_ON_WINNT
    ...
    if (nChar == VK_DOWN || nChar == VK_UP || nChar == VK_RIGHT || nChar == VK_LEFT) {
        ...
        case VK_DOWN:
            ...
            SearchSelectPosition();
            SelectCode += NUM_CHAR;
            SelectCode = CorrectEUDCCodeKey(SelectCode, NUM_CHAR, TRUE);
            SearchSelectPosition();
            this->UpdateWindow();
            break;
        case VK_UP:
...
/*    MESSAGE    "WM_SETFOCUS"        */
void CEudcList::OnSetFocus(CWnd * pOldWnd)
{
...
/*    Correct it if code is illegal     */
#ifdef BUILD_ON_WINNT
WORD CEudcList::CorrectEUDCCodeKey(WORD     Code, int     MovePt, BOOL     UporDown)
{
    COUNTRYINFO    Info;
    WORD    wCode;
...
BOOL CEudcList::IsCorrectChar(UINT     i, UINT     j)
{
    int    sViewPt;
    WORD    wCode;
    BYTE    sOffset;
...
void CEudcList::OnKillFocus(CWnd * pNewWnd)
{
    FocusFlag = FALSE;
    SearchSelectPosition();
    this->UpdateWindow();
...
void CEudcList::OnRButtonUp(UINT     nFlags, CPoint     point)
{
...
void CViewEdit::OnPaint()
{
    ...
    if (CountryInfo.bUnicodeMode) {
#ifdef UNICODE
        ExtTextOutW(dc.GetSafeHdc(), xOffset, yOffset, ETO_OPAQUE, &ViewRect, (LPCWSTR)ViewCode, 1, NULL);
#else
        CHAR ViewTmp[2];
        ...
        ExtTextOutA(dc.GetSafeHdc(), xOffset, yOffset, ETO_OPAQUE, &ViewRect, (LPCSTR)ViewCode, 2, NULL);
    }
    dc.SelectObject(OldFont);
...
/*    Draw Concave Rect        */
void CViewEdit::DrawConcave(CDC * dc, CRect     rect)
{
    CBrush    ConBrush, * OldBrush;
    CRect    Rt;
...
/*    Draw Concave Rect        */
void CCustomListFrame::DrawConcave(CDC * dc, CRect     rect)
{
    CBrush    ConBrush, * OldBrush;
    CRect    Rt;
...
/*    Draw Concave Rect        */
void CCustomInfoFrame::DrawConcave(CDC * dc, CRect     rect)
{
    CBrush    ConBrush, * OldBrush;
    CRect    Rt;
