...
//   ReverseEndian
//   Purpose: copies unicode character from one endian source to another.
...
INT FAR IsNotepadEmpty(HWND hwndParent, TCHAR* szFileSave, BOOL fNoDelete)
{
    ...
    if (!nChars) {
        if (fNoDelete)
            nRetVal = MessageBox(hwndParent, szCSEF, szNN, MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION);
...
BOOL FAR SaveFile(HWND hwndParent, TCHAR* szFileSave, BOOL fSaveAs)
{
    ...
    if (fp == INVALID_HANDLE_VALUE) {
        if (fSaveAs)
            AlertBox(hwndParent, szNN, szCREATEERR, szFileSave, MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
    }
...
BOOL FAR LoadFile(TCHAR* sz, INT typeFlag)
{
    ...
    __try {
        ...
        if (!bUnicode) {
            nChars = MultiByteToWideChar(cpTemp, 0, (LPSTR)lpBufAfterBOM, len, NULL, 0);
        }
        ...
        if (bUnicode) {
            ...
            nChars = MultiByteToWideChar(cpTemp, 0, (LPSTR)lpBufAfterBOM, len, (LPWSTR)lpch, nChars);
        }
