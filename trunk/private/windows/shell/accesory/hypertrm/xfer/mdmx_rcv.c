...
extern    int      xr_collect(ST_MDMX*, int, long, unsigned char**, unsigned char*, unsigned*);
...
int mdmx_rcv(HSESSION hSession, int attended, int method, int single_file)
{
    ...
    *fname = '\0';
    ...
    if (method == XF_YMODEM_G) {
        xc->streaming = TRUE;
        start_char = 'G';
    } else if ((method == XF_XMODEM || method == XF_XMODEM_1K) && xc->mdmx_chkt == CHECKSUM) {
        start_char = NAK;
    ...
    while (still_trying) {
        ...
        case NOBATCH_PCKT:
            ...
            mdmxdspNewfile(xc, 0, our_fname, our_fname);
        ...
        case GOOD_PCKT:
            ...
            if (xc->this_pckt == 0) {
                ...
                mdmxdspNewfile(xc, ++xc->filen, fname, our_fname);
                ...
                /* unload packet data */
                cp = last_pckt->bdata;
                xpckt_size = (last_pckt->start_char == STX ? LARGE_PACKET : SMALL_PACKET);
        ...
        case NO_PCKT:
            ...
            if (xc->mdmx_chkt == UNDETERMINED && xc->this_pckt == 0 && tries == 3
                ...
                // VidWrtStr(xc->toprow + XR_DR_ERR_CHK, xc->dc_err_chk, strld(TM_CHECKSUM));
