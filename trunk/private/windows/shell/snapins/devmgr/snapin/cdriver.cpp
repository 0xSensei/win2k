            ...
            if (INVALID_HANDLE_VALUE != hKey) {
                ...
                if (regDrv.GetValue(REGSTR_VAL_INFPATH, &regType,
                    ...
                    if (pMachine->DiSetDeviceInstallParams(*m_pDevice, &DevInstParams) &&
                        pMachine->DiBuildDriverInfoList(*m_pDevice, SPDIT_CLASSDRIVER)) {
...
BOOL CDriver::operator ==(CDriver& OtherDriver)
{
...
CDriverFile::GetVersionInfo()
{
    ...
    if (GetFileVersionInfo((LPTSTR)(LPCTSTR)m_strFullPathName, dwHandle, Size, pVerInfo)) {
        ...
        if (!VerQueryValue(pVerInfo, (LPTSTR)tszTranslation, &pBuffer, &Len)) {
            ...
            strStringFileInfo.Format(tszStringFileInfo, *((WORD*)pBuffer), *(((WORD*)pBuffer) + 1));
        }
...
BOOL CDriverFile::operator ==(CDriverFile& OtherDrvFile)
{
    return \
        ...
        (GetProvider() && OtherDrvFile.GetProvider() && !lstrcmpi(GetProvider(), OtherDrvFile.GetProvider())
         )
          ) &&
          (GetCopyright() == OtherDrvFile.GetCopyright() ||
        (GetCopyright() && OtherDrvFile.GetCopyright() && !lstrcmpi(GetCopyright(), OtherDrvFile.GetCopyright())
         )
           ) &&
           (GetVersion() == OtherDrvFile.GetVersion() ||
        (GetVersion() && OtherDrvFile.GetVersion() && !lstrcmpi(GetVersion(), OtherDrvFile.GetVersion())
         )
            );
}
