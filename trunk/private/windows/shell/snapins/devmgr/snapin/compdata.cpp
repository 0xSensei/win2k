...
ULONG CComponentData::AddRef()
{
    ::InterlockedIncrement((LONG*)&m_Ref);
    return m_Ref;
}

ULONG CComponentData::Release()
{
...
STDMETHODIMP CComponentData::QueryInterface(REFIID  riid, void** ppv)
{
    if (!ppv)
        return E_INVALIDARG;
                                    ...
                                    LPARAM param)
{
    ...
    try {
        ...
        ASSERT(m_pScope);
        INTERNAL_DATA tID;
        hr = ExtractData(lpDataObject, CDataObject::m_cfSnapinInternal, (PBYTE)&tID, sizeof(tID));
        if (SUCCEEDED(hr)) {
            switch (event) {
            case MMCN_DELETE:
...
STDMETHODIMP CComponentData::QueryDataObject(MMC_COOKIE cookie, 
...
STDMETHODIMP CComponentData::CompareObjects(LPDATAOBJECT lpDataObjectA, LPDATAOBJECT lpDataObjectB)
{
    ...
    try {
        ...
        hr = ExtractData(lpDataObjectA, CDataObject::m_cfSnapinInternal, (PBYTE)&tID_A, sizeof(tID_A));
        if (SUCCEEDED(hr)) {
            hr = ExtractData(lpDataObjectB, CDataObject::m_cfSnapinInternal, (PBYTE)&tID_B, sizeof(tID_B));
            if (SUCCEEDED(hr)) {
                hr = (tID_A.ct == tID_B.ct &&
                      tID_A.cookie == tID_B.cookie &&
...
)
{
    ...
    try {
        ...
        hr = ExtractData(lpDataObject, 
                         CDataObject::m_cfSnapinInternal,
                         reinterpret_cast<BYTE*>(&tID), 
                         sizeof(tID));
        if (SUCCEEDED(hr)) {
...
)
{
    ...
    try {
        ...
        hr = ExtractData(lpDataObject,
                         ...
                         sizeof(tID));
        if (SUCCEEDED(hr))
            hr = FindScopeItem(tID.cookie)->AddMenuItems(pCallbackUnknown, pInsertionAllowed);
    } catch (CMemoryException * e) {
        e->Delete();
        MsgBoxParam(m_hwndMain, 0, 0, 0);
        hr = E_OUTOFMEMORY;
...
STDMETHODIMP CComponentData::Command(long nCommandID, LPDATAOBJECT lpDataObject)
{
    if (!lpDataObject)
        return E_INVALIDARG;
...
HRESULT CComponentData::CreateCookieSubtree(CScopeItem* pScopeItem, CCookie* pCookieParent)
{
    ASSERT(pScopeItem);
        ...
        try {
            ...
            if (UNICODE_NULL != Info.ComputerFullName[0]) {
                ...
                WideCharToMultiByte(CP_ACP,
                                    0,
                                    Info.ComputerFullName,
                                    -1,
                                    ComputerNameA,
                                    sizeof(ComputerNameA) / sizeof(CHAR),
                                    NULL, 
                                    ULL);
...
STDMETHODIMP CComponentData::Save(IStream* pStm, BOOL fClearDirty)
{
    ...
    try {
        ...
        // assuming it is on local machine. The machine name is saved in UNICODE
...
STDMETHODIMP CComponentData::GetHelpTopic(LPOLESTR* lpCompileHelpFile)
{
...
CScopeItem* CComponentData::FindScopeItem(MMC_COOKIE cookie)
{
...
HRESULT CComponentData::LoadScopeIconsForScopePane()
{
    ASSERT(m_pScope);
    ASSERT(m_pConsole);
...
HRESULT CComponentData::DoStartupProperties(LPPROPERTYSHEETCALLBACK lpProvider,
                                            LONG_PTR handle,
                                            LPDATAOBJECT lpDataObject
)
{
...
HRESULT CComponentData::CreateScopeItems()
{
...
HRESULT CComponentData::ResetScopeItem(CScopeItem* pScopeItem)
{
    HRESULT hr = S_OK;
    if (pScopeItem) {
        CScopeItem* pChild;
