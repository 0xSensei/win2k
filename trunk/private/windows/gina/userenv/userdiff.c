...
BOOL ProcessUserDiff(LPPROFILE lpProfile, DWORD dwBuildNumber)
{
    ...
    while (lpItem) {
        ...
        if ((lpItem->dwBuildNumber > dwBuildNumber) && (lpItem->dwBuildNumber <= g_dwBuildNumber)) {
...
//              11/16/95    ericflo    Created
{
    ...
    do {
        // Query for the product type
        dwSize = sizeof(dwProductType);
        lResult = RegQueryValueEx(hKeyEntry, UD_PRODUCTTYPE, NULL, &dwType, (LPBYTE)&dwProductType, &dwSize);

        // It's ok to not have a product type listed in userdiff.ini.
        // In this case, we always apply the change regardless of the platform.
        if (lResult == ERROR_SUCCESS) {
            // A specific product was listed.  Check if we can process this entry.
            if (!OkToProcessItem(dwProductType)) {
        ...
        dwSize = MAX_PATH * sizeof(TCHAR);
        lResult = RegQueryValueEx(hKeyEntry, UD_COMMANDLINE, NULL, &dwType, (LPBYTE)szCmdLine, &dwSize);
        if (lResult != ERROR_SUCCESS) {
            goto LoopAgain;
        }
...
BOOL OkToProcessItem(DWORD dwProductType)
{
    ...
    case PT_SERVER:
        ...
        break;
    case PT_DC:
        if ((dwProductType == 0) || (dwProductType == 1) || (dwProductType == 4)) {
            bRetVal = TRUE;
        }
