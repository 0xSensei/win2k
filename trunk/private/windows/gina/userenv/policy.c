...
UINT MergeRegistryData(HKEY hkeySrc, HKEY hkeyDst, LPTSTR pszKeyNameBuffer, UINT cbKeyNameBuffer);
...
BOOL WINAPI ApplySystemPolicy (DWORD dwFlags, HANDLE hToken, HKEY hKeyCurrentUser, LPCTSTR lpUserName, LPCTSTR lpPolicyPath, LPCTSTR lpServerName)
{
    LONG lResult;
    BOOL bResult = FALSE;
    ...
    lResult = RegOpenKeyEx (HKEY_LOCAL_MACHINE, REGSTR_PATH_UPDATE, 0, KEY_READ, &hkeyMain);
    if (lResult == ERROR_SUCCESS) {
        // Look for the update mode.
        dwSize=sizeof(dwUpdateMode);
        if (RegQueryValueEx(hkeyMain,g_szUpdateModeValue,NULL,NULL, (LPBYTE) &dwUpdateMode,&dwSize) != ERROR_SUCCESS) {
                dwUpdateMode = UM_OFF;
        }
    ...
    // If we are running in automatic mode, use the supplied policy file.
    ...
    if (lResult == ERROR_SUCCESS) {
        ...
        lResult = RegQueryValueExA(hkeyLogon, g_szPolicyHandler, NULL, NULL, (LPBYTE) szHandler, &dwSize);
                ...
                *lpEntryPoint = TEXT('\0');
                ...
                if (hDLL) {
                    ...
                    if (pfn != NULL) {
                       ...
                       DebugMsg((DM_WARNING, TEXT("ApplySystemPolicy:  Failed to find entry point %S in policy dll.  Error %d."), lpEntryPoint, GetLastError()));
                    }

                    FreeLibrary(hDLL);
                } else {
                   DebugMsg((DM_WARNING, TEXT("ApplySystemPolicy:  Failed to load %S with error %d."), szHandler, GetLastError()));
                }
            }
        }
    }
    ...
    if (dwFlags & SP_FLAG_APPLY_USER_POLICY) {
        ...
        // Merge group specific policies if user name specified, and we *didn't* find a specific user entry above
        if (!fFoundUser && lpServerName && *lpServerName) {
            ...
            if (GroupBuffer) {
                // if there is a group processing order specified in policy hive, then process groups
                    ...
                    if (GetGroupProcessingOrder(hkeyMain, &GroupBuffer, &dwGroupSize)) {
                        ...
                        if (ApiBuf) {
                            ...
                            if (uEntriesRead) {
                                ...
                                // to highest priority).  for each group, if the user belongs to it then download policies for that group.
...
Exit:
    ...
    if (!DeleteFile (szLocalPath)) {
        DebugMsg((DM_WARNING, TEXT("ApplySystemPolicy:  Failed to delete policy file <%s>.  Error %d"), szLocalPath, GetLastError()));
    }

    lstrcat (szLocalPath, c_szLog);
    if (!DeleteFile (szLocalPath)) {
        DebugMsg((DM_WARNING, TEXT("ApplySystemPolicy:  Failed to delete policy log file <%s>.  Error %d"), szLocalPath, GetLastError()));
    }
...
HKEY OpenUserKey(HKEY hkeyRoot,LPCTSTR pszName, BOOL *pfFoundSpecific)
{
    ...
    *pfFoundSpecific = FALSE;
    ...
    // See if there is a subkey under the specified key with the given user name
...
UINT MergeRegistryData(HKEY hkeySrc, HKEY hkeyDst, LPTSTR pszKeyNameBuffer, UINT cbKeyNameBuffer)
{
    ...
    while ((uRet=RegEnumKey(hkeySrc,nIndex,pszKeyNameBuffer, cbKeyNameBuffer)) == ERROR_SUCCESS) {
        ...
        if ((uRet=RegCreateKey(hkeyDst,pszKeyNameBuffer, &hkeySubkeyDst)) != ERROR_SUCCESS)
                return uRet;

        if ((uRet=RegOpenKey(hkeySrc, pszKeyNameBuffer, &hkeySubkeySrc)) != ERROR_SUCCESS) {
    ...
    if (dwValueCount) {
        ...
        do  {
            ...
            if ((uRet=RegEnumValue(hkeySrc,nIndex, ValueName, &dwValueNameSize,NULL,&dwType, ValueData, &dwValueDataSize)) == ERROR_SUCCESS) {
                 ...
                 // Look for special prefixes which indicate we should treat these values specially
                 if (HasSpecialPrefix(ValueName, &dwPrefix, ValueName)) {
                     ...
                     switch (dwPrefix) {
                         ...
                         default:
                             ...
                             // This will give us less backward compatibility headaches down the road.
                     ...
                     if (uRet == ERROR_SUCCESS) {
                        switch (dwType) {
                            case REG_SZ:
                            case REG_EXPAND_SZ:
                                DebugMsg((DM_VERBOSE, TEXT("CopyKeyValues: %s => %s  [OK]"), ValueName, (LPTSTR)ValueData));
                                break;
                            case REG_DWORD:
                                DebugMsg((DM_VERBOSE, TEXT("CopyKeyValues: %s => %d  [OK]"), ValueName, (DWORD)*ValueData));
                                break;
                            default:
                                DebugMsg((DM_VERBOSE, TEXT("CopyKeyValues: %s was set successfully"), ValueName));
                        }
                     } else {
                         DebugMsg((DM_WARNING, TEXT("CopyKeyValues: Failed to set %s with error %d."), ValueName, uRet));
                     }
#endif
                 }
            }
...
BOOL GetGroupProcessingOrder(HKEY hkeyHiveRoot, LPTSTR * pGroupBuffer, DWORD * pdwGroupSize)
{
    ...
    // priority to highest.  So if we have n values, start with value name "<n>" and work down to "1".
...
LPTSTR GetUserGroups (LPCTSTR lpServerName, LPCTSTR lpUserName, HANDLE hToken, DWORD * puEntriesRead)
{
    ...
    if (!pNetAPI32) {
        DebugMsg((DM_WARNING, TEXT("GetUserGroups:  Failed to load netapi32 with %d."), GetLastError()));
        return NULL;
    }
    ...
    if (status == NERR_Success) {
        ...
        if (pNetAPI32->pfnNetUserGetInfo (NULL, lpUserName, 0, &lpTemp) == NERR_Success) {
    ...
    // API buffer to NetUserGetGroups because we're not using fixed-length fields
