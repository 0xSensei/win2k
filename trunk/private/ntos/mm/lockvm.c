...
Arguments:
   ...
   BaseAddress - The base address of the region of pages
        to be locked. This value is rounded down to the next host page address boundary.
   ...
   MapType - A set of flags that describe the type of locking to perform.  One of MAP_PROCESS or MAP_SYSTEM.
Return Value:
    Returns the status
...
--*/
{
    ...
    while (Va <= EndingAddress) {
        ...
        do {
            (VOID)MiDoesPpeExistAndMakeValid(PointerPpe, TargetProcess, FALSE, &Waited);
            Waited = 0;
            (VOID)MiDoesPdeExistAndMakeValid(PointerPde, TargetProcess, FALSE, &Waited);
        } while (Waited != 0);
        ...
        while (PointerPte->u.Hard.Valid == 0) {
            ...
            *(volatile ULONG*)Va;
            ...
            // made valid were removed from the working set before the working set lock was acquired.
            do {
                (VOID)MiDoesPpeExistAndMakeValid(PointerPpe, TargetProcess, FALSE, &Waited);
                Waited = 0;
                (VOID)MiDoesPdeExistAndMakeValid(PointerPde, TargetProcess, FALSE, &Waited);
...
Arguments:
   ProcessHandle - Supplies an open handle to a process object.
   BaseAddress - The base address of the region of pages
        to be unlocked. This value is rounded down to the next host page address boundary.
...
VOID MiInsertConflictInList(PMMLOCK_CONFLICT Conflict)
{
    ...
    InsertHeadList(&MmLockConflictList, &Conflict->List);
