...
--*/
{
    ...
    if (requestorMode != KernelMode) {
        ...
        try {
            ...
            // I/O status block.  Simply return an appropriate error status code.
    ...
    // Get the address of the current thread.  The thread contains a list of the pending operations for this file.
    ...
    // thread can only be running on one processor at a time and this routine has control of the thread for now.
    ...
    if (found) {
        ...
        // Delay execution for a time and let the request finish.  The delay time is 10ms.
        ...
        while (found) {
            ...
            // Raise the IRQL to prevent modification to the IRP list by the thread's APC routine.
            KeRaiseIrql( APC_LEVEL, &irql );
...
--*/
{
    ...
    // the close to be sent to the file system, thus causing the file to be deleted.
...
--*/
{
    ...
    // opened for asynchronous I/O.  Synchronize the completion of the I/O operation now.
    if (!synchronousIo) {
...
--*/
{
    ...
    if (requestorMode != KernelMode) {
        try {
            ...
            ProbeForWrite( FileInformation, sizeof( FILE_BASIC_INFORMATION ), sizeof( ULONG ));
        } except(EXCEPTION_EXECUTE_HANDLER) {
    ...
    // realize that it is only performing a query.  It will therefore perform the query, and immediately close the file.
    status = ObOpenObjectByName( ObjectAttributes, (POBJECT_TYPE) NULL, requestorMode, NULL, FILE_READ_ATTRIBUTES, &openPacket, &handle );

    // The operation is successful if the parse check field of the open packet
    // indicates that the parse routine was actually invoked, and the final status field of the packet is set to success.
    if (openPacket.ParseCheck != OPEN_PACKET_PATTERN) {
        return status;
    } else {
        return openPacket.FinalStatus;
