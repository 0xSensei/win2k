...
Routine Description:
    ...
    the data read from the file will be returned in the second longword of the IoStatusBlock.
Arguments:
    ...
    ByteOffset - Optionally specifies the starting byte offset within the file
        ...
        file is not opened for synchronous I/O and the parameter is not specified, then it is an error.
...
--*/
{
    ...
    // that if the caller does not have read access to the file, the operation will fail.
    ...
    if (requestorMode != KernelMode) {
        ...
        try {
            ...
            // thread.  This error will be caught by the probe/lock or buffer copy operations later.
            ...
            // two are mutually exclusive methods for I/O completion notification.
            ...
            if (ARGUMENT_PRESENT(ByteOffset)) {
                ProbeForRead(ByteOffset, sizeof(LARGE_INTEGER), sizeof(ULONG)); 
            ...
            // caller's parameters.  Dereference the file object and return an appropriate error status code.
            ObDereferenceObject(fileObject);
            return exceptionCode;
        }
    } else {
        // The caller's mode is kernel.  Get the same parameters that are required from any other mode.
        if (ARGUMENT_PRESENT(ByteOffset)) {
            fileOffset = *ByteOffset;
        }
    ...
    // I/O operation.  If it is, then wait here until the file is owned by the current thread.
    if (fileObject->Flags & FO_SYNCHRONOUS_IO) {
        ...
        if (fileObject->PrivateCacheMap) {
            ...
            if (fastIoDispatch->FastIoRead(fileObject,
                ...
                // get cache hits to be more responsive in terms of getting more CPU time.
                if (IopCacheHitIncrement) {
                    KeBoostPriorityThread(&PsGetCurrentThread()->Tcb, (KPRIORITY)IopCacheHitIncrement);
                }
    ...
    *majorFunction = IRP_MJ_READ;
    ...
    // Memory Descriptor List (MDL) is allocated and the caller's buffer is locked down using it.
    ...
    if (deviceObject->Flags & DO_BUFFERED_IO) {
        ...
        if (Length) {
            try {
                ...
                // actually happened, clean everything up, and return an appropriate error status code.
            ...
            // not be set to deallocate so the completion path does not have to special-case the length.
        ...
        if (Length) {
            try {
                ...
                // caller's buffer for write access and fill in the MDL with the PFNs of those pages.
                ...
                // happened, clean everything up, and return an appropriate error status code.
                IopExceptionCleanup(fileObject, irp, eventObject, (PKEVENT)NULL);
                return GetExceptionCode();
            }
        }
