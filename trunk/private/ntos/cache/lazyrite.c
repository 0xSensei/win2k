...
--*/
{
    ...
    //  thread could actually run and make the scan go idle before we then jam the flag TRUE.
...
Routine Description:
    ...
    writer workitem!  The caller must not be holding synchronization that could block a Cc workitem!

    In particular, this lets a caller insure that all avaliable lazy closes at the time of the call have completed.
Arguments:
    None.
Return Value:
...
--*/
{
    ...
    try {
        ...
        //  pages we have now.  Throw out any cases which would not produce a positive rate.
        ...
        while ((SharedCacheMap != FirstVisited) &&
            ...
            //  that would require additional IO already prohibited by our normal tests.
            ...
            if (!FlagOn(SharedCacheMap->Flags, WRITE_QUEUED | IS_CURSOR) &&
                ...
                //  away when we clear WRITE_QUEUED, we will find him again in the LW scan.
                if (WorkQueueEntry == NULL) {
                    CcAcquireMasterLock(&OldIrql);
                ...
                //  Make sure we occassionally drop the lock.  Set WRITE_QUEUED to keep the guy from going away.
        ...
        //  Better we bug check here than worry about what happens if we let one get by.
...
Routine Description:
    ...
    initialized by the caller, to the WorkQueue for FIFO processing by the work threads.
Arguments:
