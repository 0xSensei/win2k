...
Abstract:
    This module implements interfaces that support manipulation of memory type range registers.
...
Routine Description:
    ...
    database feature.   First processor's MTRR set is read into the physical range database.
Arguments:
...
--*/
{
    ...
    // If MTRR support disabled on first processor or if buffer not allocated then fall through
    ...
    // Ranges should be added to the range database only for one processor.
    if (Status && (KeGetPcr()->Number == 0)) {
#if IDBG
        KiDumpMTRR("Processor MTRR:", NULL);
#endif

        // Read current MTRR settings for various cached range types and add them to the range database
