...
Routine Description:
  ...
  The caller must call ShDestroyVersionStruct after the VersionStruct is no longer needed.
Arguments:
...
PCTSTR pShEnumVersionValueCommon(IN OUT  PVERSION_STRUCT VersionStruct);
PCTSTR pShEnumNextVersionTranslation(IN OUT  PVERSION_STRUCT VersionStruct);

PCTSTR pShEnumFirstVersionTranslation(IN OUT  PVERSION_STRUCT VersionStruct)
{
    UINT ArraySize;
...
BOOL pShIsDefaultTranslation(IN      PCTSTR TranslationStr)
{
    INT i;
...
PCTSTR pShEnumNextVersionValue(IN OUT  PVERSION_STRUCT VersionStruct)
{
    PCTSTR rc = NULL;
