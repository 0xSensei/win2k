...
again:
    hbm = (HBITMAP)LoadImage(HINST_THISDLL, szFile, IMAGE_BITMAP, LOWORD(cxIcon), LOWORD(cyIcon), flags);
    if (hbm == NULL)
        return 0;
...
LPVOID FindResourcePE(LPVOID pBase, LPVOID prt, int iResIndex, int ResType, DWORD * pcb)
{
    ...
    // now go find the actual resource  either by id (iResIndex < 0) or by ordinal (iResIndex >= 0)
