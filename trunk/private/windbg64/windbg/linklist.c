...
* Output:
*   Returns newly created node (zero filled) if successful, otherwise NULL.
*/
PLLE WINAPI LLPlleCreate(PLLI    plli)
{
    PLLE    plle;
    DWORD   cbNode;
...
#ifdef DEBUGTRACE
        sprintf(buf, "LLPlleCreate\tplli: %p\tplle: %p\tcbNode: %d\n", plli, plle, cbNode);
        OutputDebugString(buf);
#endif
...
*/
VOID WINAPI LLAddPlleToLl(PLLI    plli, PLLE    plle)
{
    Assert(plli);
    Assert(plle);
...
VOID WINAPI LLInsertPlleInLl(PLLI    plli, PLLE    plleNew, DWORD   lParam)
{
    DWORD           fNeedPos = TRUE;
...
*/
BOOL WINAPI LLFDeletePlleIndexed(PLLI    plli, DWORD   lPos)
{
    PLLE    plleKill;
    PLLE    pllePrev = NULL;
    DWORD   lPosCur = 0L;
...
)
{
    ...
    while (plleNext && lpfnCmp(LLLpvFromPlle(plleNext), lpv, lParam) != fCmpEQ) {
        plle = plleNext;
...
*/
DWORD WINAPI LLChlleInLl(PLLI    plli)
{
    DWORD   lRet;
...
*/
LPVOID WINAPI LLLpvFromPlle(PLLE    plle)
{
#ifdef DEBUGVER
...
*/
VOID WINAPI LLUnlockPlle(PLLE    plle)
{
    return;
}
...
*/
PLLE WINAPI LLPlleGetLast(PLLI    plli)
{
    Assert(plli);
...
#ifdef DEBUGTRACE
    sprintf(buf, "LLFRemovePlle\t plli: %p\tplle: %p\tplleCur: %p", plli, plle, plleCur);
    OutputDebugString(buf);
#endif
