...
//                              CEnumVariant object, a VARIANT enumerator that uses a SAFEARRAY to store its elements.
...
//      CEnumVariant::Create()
//      DESCRIPTION:
//              Creates an instance of the VARIANT enumerator object and initializes it.
//      PARAMETERS:
...
HRESULT CEnumVariant::Create(SAFEARRAY FAR * psa, ULONG cElements, CEnumVariant FAR * FAR * ppenumvariant)
{
    HRESULT                             hr;
    ...
    *ppenumvariant = NULL;
    ...
    //    of the SAFEARRAY in-parameter.  This will be used as the lower bound for the SAFEARRAY member.
    hr = SafeArrayGetLBound(psa, 1, &lLBound);
    if (FAILED(hr))
        goto error;

    //  Initialize the SAFEARRAY member of the enumerator by copying the SAFEARRAY in-parameter.
    ...
    *ppenumvariant = penumvariant;
    ...
    //  If an error occurs, ...
error:
    //  If the enumerator creation failed, return E_OUTOFMEMORY.
    if (penumvariant == NULL)
...
//      DESCRIPTION:
//              Object constructor.  Initializes m_psa, the SAFEARRAY pointer, to NULL.
CEnumVariant::CEnumVariant()
{
    m_psa = NULL;
}
...
STDMETHODIMP CEnumVariant::QueryInterface(REFIID iid, void FAR * FAR * ppv)
{
    if (!ppv)
        return E_INVALIDARG;
...
//              Decrements the reference count, deleting the object when the reference count reaches zero.
//      RETURNS:
//              ULONG                   Current reference count.
...
STDMETHODIMP CEnumVariant::Next(ULONG cVars, VARIANT FAR * rgVar, ULONG FAR * pcVarsFetched)
//      CEnumVariant::Next()
//      DESCRIPTION:
//              Implements the IEnumVARIANT interface method Next().
//              Attempts to retrieve the next cVars VARIANTs in the
//                      enumeration sequence and return them through the array pointed to by rgVar.
//      PARAMETERS:
//              cVars                           [in] The number of VARIANTs to be returned.
//              rgVar                           [in,out] An array of at least size cVars
//                                                               in which the VARIANTs are to be returned.
//              pcVarsFetched           [out] Pointer to the number of VARIANTs returned in rgVar.
//      RETURNS:
//              HRESULT                 S_OK if exactly cVars VARIANTS were retrieved.
//                                      S_FALSE if less than cVars VARIANTS were retrieved.
//                                      E_INVALIDARG if the parameters are invalid.
{
    ...
    *pcVarsFetched = 0;//  Initialize pcVarsFetched.
    ...
    *pcVarsFetched = l2;//  Set the count of the VARIANTs retrieved.    
    m_lCurrent = l1;//  Update the current enumerator element.

    //  Return either S_OK or S_FALSE depending on how many VARIANTs were actually retrieved.
    return (l2 < cVars) ? S_FALSE : S_OK;
    
error://  On error, ...
    //  Clear all the VARIANTs in rgVar and return the error code.
    for (l = 0; l < cVars; l++)
...
STDMETHODIMP CEnumVariant::Clone(IEnumVARIANT FAR * FAR * ppenum)
{
    ...
    *ppenum = NULL;
    ...
    //  Create a new CEnumVariant with this CEnumVariant's SAFEARRAY and count of elements.
    ...
    //  Initialize the current VARIANT index of the new CEnumVariant from the index of this CEnumVariant.
    penum->m_lCurrent = m_lCurrent;

    //  Point the out-parameter at the new enumerator and return success.
    *ppenum = penum;
