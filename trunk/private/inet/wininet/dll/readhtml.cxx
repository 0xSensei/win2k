...
BOOL ReadHtmlUrlData(IN HINTERNET hInternet,
                     IN LPVOID lpBuffer,
    ...
    *lpdwBytesReturned = dwBufferLength;
    ...
    do {
...
#else
            if ((handleType == TypeFtpFileHandleHtml) || (handleType == TypeGopherFileHandleHtml))
#endif
            {
        ...
        case HTML_STATE_HEADER:
        {
            ...
            // case this is an FTP request and we need to add a root directory path
            ...
            charsCopied = (DWORD)wsprintf((LPSTR)scratchPad, HTML_DOCUMENT_HEADER, title, title);
            if (charsCopied > dwBufferLength) {
                // caller's buffer not large enough to fit header - clean up
                // and allow the caller to retry
                success = FALSE;
                ...
                if (handleType == TypeFtpFindHandleHtml) {
                    if (urlLength == 0) {
                        *url = '/';
                        ++urlLength;
                    } else if ((url[urlLength - 1] != '/') && (url[urlLength - 1] != '\\')) {
                        url[urlLength++] = '/';
                    }
                    url[urlLength] = '\0';
                } else {
        ...
        case HTML_STATE_WELCOME:
        {
            ...
            if (handleType == TypeGopherFindHandleHtml && ((GOPHER_FIND_HANDLE_OBJECT*)hInternet)->GetFixedType()) {
                ...
                if (((GOPHER_FIND_HANDLE_OBJECT*)hInternet)->GetFixedType() == GOPHER_TYPE_CSO) {
                    success = TRUE;
                    dataCopied = FALSE;
            ...
            pConnect = (INTERNET_CONNECT_HANDLE_OBJECT*)((HANDLE_OBJECT*)hInternet)->GetParent();
            ...
            if ((handleType == TypeFtpFindHandleHtml) && (lastInfo != NULL)) {
                ...
                // "230-", or each may start with a line beginning only with a space
                ...
                if (p != NULL) {
                    ...
                    if (bufferLeft >= (sizeof(HTML_FTP_WELCOME_START) - 1)) {
                        memcpy(pBuffer, HTML_FTP_WELCOME_START, sizeof(HTML_FTP_WELCOME_START) - 1);
                    ...
                    // then copy each welcome message line, dropping the "230-" from each
                    do {
                        if (!strncmp(p, FTP_WELCOME_INTRO, FTP_WELCOME_INTRO_LENGTH)) {
                            p += FTP_WELCOME_INTRO_LENGTH;
                        } else if (*p != ' ') {
                            // line doesn't start with "230-" or a space. We're done
                            break;
                        }
                    ...
                    if (bufferLeft >= (sizeof(HTML_FTP_WELCOME_END) - 1)) {
                        memcpy(pBuffer, HTML_FTP_WELCOME_END, sizeof(HTML_FTP_WELCOME_END) - 1);
        ...
        case HTML_STATE_DIR_HEADER:
            ...
            // encapsulated. This is no longer true, so I could change things somewhat

            // if FTP directory AND not root directory, display link to higher level
            DWORD cbTotal;
            char szUp[64];
            LPSTR lpszHtml;
            ...
            if (handleType == TypeFtpFindHandleHtml) {
                ...
                if ((*url != '\0') && (memcmp(url, "/", 2) != 0)) {
                    cbTotal += LoadString(GlobalDllHandle, IDS_FTP_UPLEVEL, szUp, sizeof(szUp));
                }
            } else {
        ...
        case HTML_STATE_BODY:
            ...
            case TypeFtpFindHandleHtml:
                success = FtpFindNextFileA(hInternet, (LPWIN32_FIND_DATA)dirBuffer);
            ...
            create_dir_entry:
                if (success) {
                    ...
                    if (error != ERROR_SUCCESS) {
                        ...
                        // entry then store it in the handle object for next time
                        if (lastDirEntry != NULL) {
                            RSetDirEntry(hInternet, lastDirEntry);

                            // no longer need our copy of the string -
                            // RSetLastDirEntry() also makes a copy and adds it to the handle object
                    ...
                    // change the error to success - ReadFile() doesn't return ERROR_NO_MORE_FILES
                    SetLastError(ERROR_SUCCESS);

                    // reached the end of the directory listing - time to add the footer
                    charsCopied = 0;
                    success = TRUE;
                    htmlState = (HTML_STATE)((int)htmlState + 1);
                }
            ...
            case TypeGopherFileHandleHtml:
                ...
                success = InternetReadFile(hInternet, (LPVOID)buffer, dwBufferLength, &charsCopied);
                if (success) {
                    if (charsCopied == 0) {
                        // read all of file - we're done (no HTML to add for files)
        ...
        case HTML_STATE_DIR_FOOTER:
            if (dwBufferLength >= sizeof(HTML_DOCUMENT_DIR_END) - 1) {
                memcpy(buffer, HTML_DOCUMENT_DIR_END, sizeof(HTML_DOCUMENT_DIR_END) - 1);
        ...
        case HTML_STATE_FOOTER:
            // copy the HTML document footer - don't copy the end-of-string character ('\0')
            if (dwBufferLength >= sizeof(HTML_DOCUMENT_FOOTER) - 1) {
                memcpy(buffer, HTML_DOCUMENT_FOOTER, sizeof(HTML_DOCUMENT_FOOTER) - 1);
...
quit:
    ...
    // if we succeeded then return the amount of data read, else reset the last error
    if (error == ERROR_SUCCESS) {
        *lpdwBytesReturned -= dwBufferLength;
    } else {
        *lpdwBytesReturned = 0;
...
Return Value:
    DWORD
        ...
        Failure - ERROR_INTERNET_INCORRECT_HANDLE_TYPE
                    Can only handle TypeFtpFindHandleHtml and TypeGopherFindHandleHtml
...
--*/
{
    ...
    case TypeFtpFindHandleHtml:
        error = ((FTP_FIND_HANDLE_OBJECT*)hInternet)->QueryHtmlDataAvailable(lpdwNumberOfBytesAvailable);
        break;
#ifdef EXTENDED_ERROR_HTML
    case TypeFtpFileHandleHtml:
        error = ((FTP_ERROR_HANDLE_OBJECT*)hInternet)->QueryHtmlDataAvailable(lpdwNumberOfBytesAvailable);
        break;
#endif
    case TypeGopherFindHandleHtml:
        error = ((GOPHER_FIND_HANDLE_OBJECT*)hInternet)->QueryHtmlDataAvailable(lpdwNumberOfBytesAvailable);
        break;
    default:
...
Arguments:
    ...
    DirEntry        - if we couldn't copy the directory entry to the HTML buffer
                      ...
                      it the next time the function is called, before getting the next dir entry
Return Value:
    DWORD
        Success - ERROR_SUCCESS
...
--*/
{
    ...
    if (isDir) {
        // directories show up as DOS-style <DIR> entries, except we use the full "Directory" text
        ...
        // if this is a gopher item and it is an index server (7) then indicate the fact using this string
        if (!szSearch[0]) {
    ...
    nPrinted = wsprintf(entryBuf,
                        isDir ? HTML_DOCUMENT_DIR_ENTRY : HTML_DOCUMENT_FILE_ENTRY,
                        timeBuf,
                        pSizeDir,
                        url,
                        entryName);
