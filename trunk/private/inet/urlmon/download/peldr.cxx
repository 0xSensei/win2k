...
HRESULT GetMachineTypeOfFile(const char * szName, LPDWORD pdwMachine)
{
    ...
    *pdwMachine = IMAGE_FILE_MACHINE_UNKNOWN;
    ...
    // Read DOS header
    if ((!ReadFile(dfhFile, &idh, sizeof(idh), &dwBytesRead, NULL)) || (idh.e_magic != 0x5a4d)) {
        // not PE file!
        hr = HRESULT_FROM_WIN32(GetLastError());
        if (SUCCEEDED(hr)) {
...
HRESULT IsCompatibleType(DWORD dwBinaryType)
{
...
HRESULT IsCompatibleFile(const char * szFileName, LPDWORD lpdwMachineType)
{
    DWORD dwMachine = 0;
...
HRESULT IsRegisterableDLL(const char * szFileName)
{
    ...
    if ((dfhFile = CreateFile(szFileName,
                              GENERIC_READ,
                              FILE_SHARE_READ,
                              NULL,
                              OPEN_EXISTING,
...
BOOL WINAPI GetRVASectionHdr(LPVOID   lpFile, DWORD    dwRVA, IMAGE_SECTION_HEADER * sh)
{
    PIMAGE_OPTIONAL_HEADER   poh = (PIMAGE_OPTIONAL_HEADER)OPTHDROFFSET(lpFile);
    PIMAGE_SECTION_HEADER    psh = (PIMAGE_SECTION_HEADER)SECHDROFFSET(lpFile);
    int              nSections = NumOfSections(lpFile);
    ...
    /* locate section containing image directory */
    while (i++ < nSections) {
        if (psh->VirtualAddress <= dwRVA && psh->VirtualAddress + psh->SizeOfRawData > dwRVA)
            break;
        psh++;
    }
...
BOOL WINAPI ImageDirectorySectionHdr(LPVOID   lpFile, DWORD    dwIMAGE_DIRECTORY, IMAGE_SECTION_HEADER * sh)
{
    PIMAGE_OPTIONAL_HEADER   poh = (PIMAGE_OPTIONAL_HEADER)OPTHDROFFSET(lpFile);
    PIMAGE_SECTION_HEADER    psh = (PIMAGE_SECTION_HEADER)SECHDROFFSET(lpFile);
    int              nSections = NumOfSections(lpFile);
